    ...
    def train(self, train_loader):
        ...
        for epoch in range(self.epoch):
            for step, train_batch in enumerate(train_loader):
                ...
                # token_output.shape = (batch_size, max_length, token_num_labels)
                active_logits = token_output.view(-1, token_output.shape[2])[active_loss]
                # token_label_ids.shape = (batch_size, max_length)
...
def main():
    ...
    args.device = device
    tokenizer = BertTokenizer.from_pretrained(args.bert_dir, padding="max_length", max_length=512, return_tensors="pt")
    ...
    model.to(device)
    trainer = Trainer(model, args, tokenizer)
...
def test_func():
    ...
    if args.do_train:
        ...
        for i in train_loader:
            ...
            print(i['token_label_ids'].shape)
            print(i['seq_label_ids'].shape)
